...
for i in range(1, rank_pca+1):
    if (i == 1) or ((i % interval) == 0):
        im = axes.flat[subplot_counter].imshow(np.loadtxt(output_path + "arrays_matrix/everysecond_planets_pca_" + str(i) + ".txt"), origin='lower', vmin=vmin, vmax=vmax, extent=[size, -size, -size, size])
...
for init in rank_ipca_init_list:
    ...
    for end in range(init+1, rank_ipca_end+1):
        if (end == 1) or ((end % interval) == 0):
            im = axes.flat[subplot_counter].imshow(np.loadtxt(output_path + "arrays_matrix/everysecond__planets_ipca_" + str(init) + "_" + str(end) + ".txt"), origin='lower', vmin=vmin, vmax=vmax, extent=[size, -size, -size, size])
...
plt.savefig(output_path +"plots/everysecond_matrix.png", bbox_inches='tight', orientation="landscape", dpi=1000)
